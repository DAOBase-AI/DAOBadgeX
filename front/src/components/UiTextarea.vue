<template>
  <div>
    <div v-if="!!label" class="text-black flex mb-3">
    ...
    <div>
      <div
        class="relative flex min-h-10 ui-input--wrapper"
        :class="{
      ...
      >
        <div
          v-if="readonly"
          class="flex-1 flex items-center text-whitebase text-sm pl-4 font-bold"
          :class="{
            uppercase: uppercase,
          }"
          style="color: #2a2e33"
        ...
        <div v-else class="relative flex-1 flex">
          <textarea
            ...
            :disabled="disabled"
            @change="onInput"
            class="flex-1 min-w-0 ui-input text-whitebase text-sm"
            :class="{
              uppercase: uppercase,
            }"
            :rows="props.rows"
          ...
          <div
            class="absolute bottom-1 right-1 text-xs flex flex-col items-end"
          >
            <transition name="fade">
              <div
                v-if="!!state.text && state.focused"
                class="w-4 h-4 rounded-full flex items-center justify-center cursor-pointer"
            ...
            <div v-if="props.maxLength != 100000">
              {{ state.text.length }}/{{ props.maxLength }}
            </div>
          </div>
        </div>

        <slot name="trailing">
          <div v-if="endTag" class="text-black text-sm flex items-center px-2">
            {{ endTag }}
          </div>
        </slot>
      </div>
...
const checkRule = (val) => {
  ...
  if (words === '' || words == undefined) {
    return props.required ? "Can't be empty" : '';
  }
  ...
  if (words.length < props.minLength) {
    return `Min ${props.minLength} characters`;
  }
  if (props.maxValue && +words > props.maxValue) {
    return `No more than ${props.maxValue} ${props.endTag}`;
  }
  return '';
};
