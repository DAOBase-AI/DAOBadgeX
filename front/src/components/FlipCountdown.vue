      ...
      <div class="flex items-center">
        <span class="flip-clock__piece" :id="data.elementId" v-show="data.show">
          <div v-if="flipAnimation">
            ...
            <div v-else style="font-weight: bold; color: #ff2640">
              {{ data.current }} Days
            </div>
          </div>
          <div v-else>
            <span class="no-animation__card">{{
              twoDigits(data.current)
            }}</span>
          </div>
          <span
            v-if="showLabels"
        ...
        <div
          class="text-whitebase mx-2"
          v-if="data.label == 'Minutes' || data.label == 'Hours'"
        >
          :
        </div>
      </div>
...
export default {
  ...
  props: {
    deadline: {
      type: String,
      required: false,
      default: dayjs().add(32, 'd').add(10, 's').format(fmt),
    },
    deadlineISO: {
      type: String,
      required: false,
